...
// ============================================================
// SOSYAL SİSTEMLER: CHAT + WEBRTC SES + ANA MENÜ
// Mevcut online oyun state'i değiştirilmeden ek katman olarak çalışır.
// ============================================================
let chatOpen = false;
...
function toggleChat(force) {
    ...
    if (button) button.classList.toggle('chat-active', chatOpen);
    if (chatOpen) {
...
function addChatMessage(message) {
    ...
    if (message.system) {
        ...
        name.textContent = String(message.name || 'Oyuncu');
        item.appendChild(name);
        item.appendChild(document.createTextNode(': ' + String(message.text || '')));
    }
...
function submitChatMessage() {
    ...
    if (!text) return;
    if (!room) {
        setStatus('Önce bir odaya katılmalısınız.');
        return;
    }
    socket.emit('chatMsg', text.slice(0, 500));
    input.value = '';
    input.focus();
}
...
function updateVoiceUI(state, text) {
    ...
    if (button) button.classList.toggle('voice-active', state === 'on');
    if (icon) icon.textContent = state === 'on' ? '🎙️' : '🎙';
    if (label) label.textContent = state === 'on' ? 'Mikrofon Açık' : 'Sesli Sohbet';
    if (status) status.hidden = state !== 'on' && state !== 'loading';
    if (statusText) statusText.textContent = text || (state === 'on' ? 'Sesli sohbet açık' : '');
}
...
function closeVoicePeer(peerId) {
    ...
    if (peer.audio && peer.audio.parentNode) peer.audio.parentNode.removeChild(peer.audio);
...
function attachVoiceAudio(peerId, stream) {
    let peer = voicePeers.get(peerId);
    if (!peer) return;
    if (!peer.audio) {
    ...
    const playPromise = peer.audio.play();
    if (playPromise && playPromise.catch) playPromise.catch(() => {});
}

async function createVoicePeer(peerId, initiator) {
    if (!peerId || peerId === myId || voicePeers.has(peerId)) return;
    if (!voiceEnabled && !localVoiceStream) return;
...
async function handleVoiceSignal(from, signal) {
    ...
    if (!entry) {
        // Gelen offer ile bağlantı kuruluyorsa mikrofon açık olmasa bile
        // karşı tarafın sesini dinleyebiliriz.
        ...
        pc.ontrack = event => {
            const stream = event.streams && event.streams[0];
            if (stream) attachVoiceAudio(from, stream);
        };
        pc.onconnectionstatechange = () => {
            if (['failed', 'closed', 'disconnected'].includes(pc.connectionState)) closeVoicePeer(from);
        };
    }
        ...
        if (signal.type === 'offer') {
            ...
            if (localVoiceStream) {
                localVoiceStream.getTracks().forEach(track => {
                    const alreadyAdded = entry.pc.getSenders().some(sender => sender.track && sender.track.id === track.id);
                    if (!alreadyAdded) entry.pc.addTrack(track, localVoiceStream);
                });
            }
...
async function toggleVoiceChat() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        updateVoiceUI('off', 'Tarayıcı sesli sohbeti desteklemiyor.');
        setStatus('Bu tarayıcıda mikrofon erişimi desteklenmiyor.');
        return;
    }

    updateVoiceUI('loading', 'Mikrofon izni bekleniyor...');
    try {
        localVoiceStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
        voiceEnabled = true;
        updateVoiceUI('on', 'Sesli sohbet açık');
        socket.emit('voiceJoin');

        const humans = room.players
            .filter(p => !p.bot && p.id !== myId)
            .map(p => p.id);
        for (const peerId of humans) {
            await createVoicePeer(peerId, String(myId) < String(peerId));
        }
    } catch (err) {
        voiceEnabled = false;
        if (localVoiceStream) localVoiceStream.getTracks().forEach(track => track.stop());
        localVoiceStream = null;
        updateVoiceUI('off', 'Mikrofon izni verilmedi.');
        setStatus('Mikrofon erişimi verilmedi.');
    }
}

function stopVoiceChat() {
    if (voiceEnabled) socket.emit('voiceLeave');
    voiceEnabled = false;
    closeAllVoicePeers();
    if (localVoiceStream) {
    ...
    updateVoiceUI('off', 'Sesli sohbet kapalı');
}
...
function returnToMainMenu() {
    ...
    onlineReady = false;
    mySeatIndex = null;
    gameBindingsReady = false;
    lastHandIds = [];
    lastDiscardIds = [null, null, null, null];
    document.getElementById('chat-messages').innerHTML = '';
    showLobby();
    setStatus('Oyun menüsü hazır.');
...
function setupSocialUI() {
    ...
    socket.on('chatMsg', message => {
        ...
        if (message.system) setStatus(message.text || '');
    });

    socket.on('voicePeers', peers => {
        if (!voiceEnabled) return;
        peers.forEach(peerId => {
            createVoicePeer(peerId, String(myId) < String(peerId));
        });
    });
    ...
    socket.on('voicePeerLeft', peerId => closeVoicePeer(peerId));
    socket.on('voiceSignal', ({ from, signal }) => handleVoiceSignal(from, signal));

    socket.on('connect', () => {
        // Bağlantı yenilendiyse sosyal durum sıfırlanır.
        updateVoiceUI('off', 'Sesli sohbet kapalı');
    });
}

window.addEventListener('load', setupSocialUI);
